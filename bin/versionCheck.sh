...
if [ "$IS_PULL_REQUEST" != "false" ]; then
    if git diff --quiet "origin/${TARGET_BRANCH}...HEAD" 'github_approval_checker' "test" setup.* ./*.pip; then
        echo "No changes found to main code or dependencies: no version change needed"
        exit 0
    fi

    CURRENT_VERSION=$(git show "origin/${TARGET_BRANCH}:github_approval_checker/version.py" | sed -n 's/^__version__ = "\(.*\)"$/\1/p')
    NEW_VERSION=$(sed -n 's/^__version__ = "\(.*\)"$/\1/p' github_approval_checker/version.py)
